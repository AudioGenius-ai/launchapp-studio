...
pub fn validate_required_field(value: &Option<String>, field_name: &str) -> Result<()> {
    match value {
        Some(v) if !v.trim().is_empty() => Ok(()),
        _ => Err(ProjectManagementError::ValidationError {
            field: field_name.to_string(),
            message: "Field is required and cannot be empty".to_string(),
        }),
    }
}
...
pub fn validate_string_length(value: &str, field_name: &str, min: usize, max: usize) -> Result<()> {
    if value.len() < min {
        return Err(ProjectManagementError::ValidationError {
            field: field_name.to_string(),
            message: format!("Field must be at least {} characters", min),
        });
    }
    ...
    if value.len() > max {
        return Err(ProjectManagementError::ValidationError {
            field: field_name.to_string(),
            message: format!("Field must be no more than {} characters", max),
        });
    }
...
pub fn validate_file_size(size: u64, max_size: u64) -> Result<()> {
    if size > max_size {
        return Err(ProjectManagementError::FileSizeExceeded { size });
    }
    Ok(())
}

pub fn validate_mime_type(mime_type: &str, allowed_types: &[&str]) -> Result<()> {
    if !allowed_types.contains(&mime_type) {
        return Err(ProjectManagementError::UnsupportedFileType {
